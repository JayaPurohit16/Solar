    ...
    public function store(Request $request)
    {
        try {
            ...
            if ($request->hasFile('logo')) {
                if ($cms->logo != null) {
                    if (File::exists(public_path('Cms/Logo/' . $cms->logo))) {
                        unlink(public_path('Cms/Logo/' . $cms->logo));
                    }
                }
                $file = $request->file('logo');
                $extension = $file->getClientOriginalExtension();
                $filename = uniqid() . '.' . $extension;
                $file->move('Cms/Logo', $filename);
                $cms->logo = $filename;
            }

            if ($request->hasFile('footer_logo')) {
                if ($cms->footer_logo != null) {
                    if (File::exists(public_path('Cms/FooterLogo/' . $cms->footer_logo))) {
                        unlink(public_path('Cms/FooterLogo/' . $cms->footer_logo));
                    }
                }
                $file = $request->file('footer_logo');
                $extension = $file->getClientOriginalExtension();
                $filename = uniqid() . '.' . $extension;
                $file->move('Cms/FooterLogo', $filename);
